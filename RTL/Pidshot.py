...
def Pidshot(x,y,vehicle):
    ...
    if (current_X<=42 or current_X>=98):
        # 计算误差
    ...
    if (current_Y<=25 or current_Y>=81):
        # 计算误差
        ...
        error_priorY = errorY
        # 应用控制量到无人机
        send_body_ned_velocity(0,controlY*0.1,vehicle)
        # 测量当前Y坐标
        current_Y=y/480*105    

    if (current_X>=42 and current_X<=98 and current_Y>=25 and current_Y<=81):
